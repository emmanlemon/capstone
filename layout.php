...
include "db_conn.php";
 ...
 // Create database connection
  ...
  $foridandname = mysqli_query($db, "SELECT * FROM users WHERE fname='$id' OR id='$id'");
  			...
  			$result8 = mysqli_query($db, "SELECT * FROM posts ORDER BY post_id desc");
				    ...
				    <div id="headgreet" style="float: left; width: 20vw; margin-left: 20px;">
					    ...
					    <div class="sound3 searchboox" style="display: inline-block; position: relative; width: 40%; float: left; padding-left: 180px;">
					    	<form method="post" id="formforsearch" action="search.php" style="margin-top: 5px;">
					    		<input type="search" name="search" placeholder="Type the id or firstname of the person you want to find" style="width: 100%; border-radius: 10px; height: 50px; position: relative; font-size: 15px; display: inline-block; min-width: 200px; background-color: white; color: black; font-weight: bold;">
		        ...
		        <div id="contactarea" class="contact">
					<div class="postarea" style="float: left; margin-top: -110px;">
						<form method="POST" action="postupload.php" style="width: 50%; margin: 20px auto;">
									  	...
									  	<center>
									  	<div>
									  		<button class="sound3" type="submit" id="feedbackpost" name="postit" style="font-size: 30px; margin-top: -10px; float: right; z-index: 2; border-radius: 15px; background-image: url('photos/bg2.jpg'); color: white; border: solid white 1px; opacity: 0.9; cursor: pointer;">POST</button>
									  	</div>
									  </center>
						...
						<center>
							<div id="containposts" class="containposts" style="margin-top: -50px; background-color:  rgba(255,255,255,0.3); float: right; margin-right: 10px; box-shadow: 0 22px 26px 0 rgba(255 ,255 , 255,0.5), 0 17px 20px 0 rgba(255 ,255 , 255,0.4);">
								<?php
									echo "<style>
											.lalagyanngposts {
												width: 4200px;
								    ...
								    while ($row = mysqli_fetch_array($result8)) {
								      echo "<tr style='display: block; margin-bottom: 50px;'><td class='lalagyanngoras'>".$row['post_id']."</td>";
								      	echo "<td class='sound3 lalagyanngposts' style='color: white; margin-bottom: 50px;'>".$row['header']."<img src='gallery/".$row['thumbnail']."'  style='width: 80px; height: 80px; background-repeat: no-repeat; background-size: 80px 80px; background-position: center; cursor: pointer;'><img src='gallery/".$row['fimage']."'  style='width: 80px; height: 80px; background-repeat: no-repeat; background-size: 80px 80px; background-position: center; cursor: pointer;'><img src='gallery/".$row['simage']."'  style='width: 80px; height: 80px; background-repeat: no-repeat; background-size: 80px 80px; background-position: center; cursor: pointer;'></td>";
