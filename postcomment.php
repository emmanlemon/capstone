...
<html>
    ...
    <body>
								<?php
                                    include "db_conn.php";
                                    $postid = $_GET['postid'];
                                    $result9 = mysqli_query($db, "SELECT * FROM comments WHERE post_id=$postid ORDER BY comment_id desc");
									echo "<style>
											.lalagyanngposts {
												width: 120vw;
												height: 230px;
										 ...
										 </style><table style='margin-right: 4vw;'>";
								    while ($row = mysqli_fetch_array($result9)) {
								      echo "<tr style='display: block; margin-bottom: 50px;'><td class='lalagyanngoras'>Post ID: ".$row['post_id']. " &nbsp;&nbsp;Date: " .$row['timeuploaded']."" ."</td>";
								      	echo "<td onclick=\"document.location='document-view.php?post_id=".$row['post_id']."'\" class='sound3 lalagyanngposts' style='color: white; margin-bottom: 50px; padding: 7%; padding-left: 3%;'>
                                                <div style='float: left;'>
                                                    <img src='gallery/".$row['post_text']."'  style='background-color: white; width: 280px; height: 280px; background-repeat: no-repeat; background-size: cover; background-position: center; cursor: pointer; border-radius: 5px;'></div>
